...
Auto task tracking (`checklist.rs`, the only module with unit tests): `parse_checklist` reads GitHub-style `- [ ]`/`- [x]` items, **section-aware** — `##` headings group items into `ChecklistSection`s, and a section whose heading contains a backlog keyword (`backlog`/`later`/`someday`/`ideas`/`future`/`icebox`/`wishlist`) is parsed/shown but **excluded from the headline `total()`/`done()`**. The parser also captures surrounding prose: the file **preamble** (between the `#` title and the first `##`, `_Status_` line excluded, leading `>` stripped) → `Checklist::description` (the project description); per-section prose before its first item → `ChecklistSection::description`; and an indented non-checkbox line directly beneath an item → `ChecklistItem::detail` (a blank line ends the detail block). Sections are kept **flat** but carry a heading `level` (`##`=2, `###`=3, …) so the UI renders hierarchy — a deeper section is a sub-phase of the nearest shallower one, indented in the Roadmap tab. A header-only umbrella (no items) is kept when it has a description. **Backlog is inherited**: a section nested under a backlog heading is itself uncounted (a `(level, counted)` stack in `parse_sections`), even if its own title lacks a backlog keyword. The canonical source is a project-root **`ROADMAP.md`**; fallback order is `ROADMAP.md` → `ROADMAP` → `TODO.md` → `TODO` → `README.md` (first with checkboxes wins). Only that one file is parsed — `docs/` design docs are deliberately ignored to avoid counting non-task checkboxes. The convention is specified in `docs/roadmap-format.md`, and ProjectShelf's own `ROADMAP.md` is the reference example. It is **read-only** (the file is the source of truth; never written back). `build_project` caches headline counts in `TaskProgress` on `Project` (`task_total`/`task_done`/`task_source` columns); the full sectioned list is re-parsed on demand and cached in `app.rs`'s `task_cache` (invalidated on scan/watch). UI: a `☑ done/total` badge in list rows + a dedicated **Roadmap tab** in the details panel (`render_roadmap`) — progress bar (done/total + %) above an independently-scrollable item list (sections as headings, done items struck-through, backlog labeled, plus section descriptions and per-item detail as muted text). This replaced the old manual milestones feature.

The **project description** shown at the top of the Overview tab is the roadmap preamble (`Checklist::description`, read-only). For projects whose roadmap has no preamble, it falls back to an editable box stored in `user_meta.description` (DB column + `ProjectFile.description` in the YAML, synced via `merge_yaml_meta` on scan/watch and written by `save_project_file`, mirroring how `notes` works).

The details panel (`render_project_details`) has a header (name/path, action buttons, Pin) above a tab bar driven by `DetailTab` (`detail_tab` field): **Overview** (`render_overview` — description at top, then Activity + Health side-by-side via `ui.columns`, Languages, Export, a collapsed Tags editor), **Roadmap**, and **Notes**. The whole panel is wrapped in a `ScrollArea`.
